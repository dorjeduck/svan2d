...
class KeystateBuilder:
    ...
    def state_interpolation(
        self: T,
        fn: Callable,
        at: tuple[float, float] = (0.0, 1.0),
    ) -> T:
        """Define an element entirely via a state_interpolation function.

        The function is the sole source of truth for all t values including
        boundaries. Keystates are auto-generated by calling fn(None, None, t)
        at the start and end times.

        Args:
            fn: Callable (start_state, end_state, t) -> State. Called for ALL t
                values including exact boundary times. start_state and end_state
                are the auto-generated boundary states (or None on first call).
            at: Time range as (start, end). Defaults to (0.0, 1.0).

        Returns:
            New instance with keystates and transition configured.
        """
        if self._builder is None:
            raise RuntimeError("Cannot modify element after rendering has begun.")

        start_state = fn(None, None, 0.0)
        end_state = fn(None, None, 1.0)

        return (
            self.keystate(start_state, at=at[0])
            .transition(state_interpolation=fn, covers_boundaries=True)
            .keystate(end_state, at=at[1])
        )
