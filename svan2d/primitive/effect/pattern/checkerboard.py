...
class CheckerboardPattern(Pattern):
    ...
    def to_drawsvg(self, drawing: dw.Drawing | None = None) -> dw.Pattern:
        ...
        from svan2d.primitive import RectangleRenderer, RectangleState
        ...
        tl_state = RectangleState(
            pos=Point2D(self.square_size / 2, self.square_size / 2),
        ...
        br_state = RectangleState(
            pos=Point2D(self.square_size * 1.5, self.square_size * 1.5),
            width=self.square_size,
            height=self.square_size,
            fill_color=self.color1,
        )
        ...
        tr_state = RectangleState(
            pos=Point2D(self.square_size * 1.5, self.square_size / 2),
            width=self.square_size,
            height=self.square_size,
        ...
        bl_state = RectangleState(
            pos=Point2D(self.square_size / 2, self.square_size * 1.5),
            width=self.square_size,
            height=self.square_size,
            fill_color=self.color2,
        )
