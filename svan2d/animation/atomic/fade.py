...
def fade(
    ...
    """Fade out first state, switch attributes, fade in second state

    Creates a crossfade effect on a single element by fading to transparent,
    switching attributes, then fading back to opaque.
    ...
    Example:
        >>> from svan2D.animations.atomic import fade
        >>>
    ...
    keystates = [
        ...
        KeyState(
            time=at_time, state=replace(state2, opacity=0.0)
        ),  # Switch (invisible)
