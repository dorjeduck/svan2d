...
def serve(file: Path, port: int, frames: int, fps: int, no_browser: bool):
    ...
    auto_open = not no_browser and config.get(
        ConfigKey.DEVSERVER_AUTO_OPEN_BROWSER, True
    )
    ...
    from svan2d.server.dev.server import create_app
    ...
    # Create server
    click.echo(f"Starting Svan2D development server...")
