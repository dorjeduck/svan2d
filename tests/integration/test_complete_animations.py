...
class TestBasicAnimations:
    ...
    def test_multi_keystate_animation(self):
        ...
        states = [
            ...
            CircleState(Point2D(x=50, y=50), radius=75),
...
class TestSceneComposition:
    ...
    def test_scene_with_single_element(self):
        ...
        state = CircleState(Point2D(), radius=50)
    ...
    def test_scene_with_multiple_elements(self):
        ...
        circle = VElement(state=CircleState(pos=Point2D(-100, 0), radius=50))
        rect = VElement(state=RectangleState(pos=Point2D(100, 0), width=100, height=60))
    ...
    def test_scene_with_animated_elements(self):
        ...
        states1 = [
            CircleState(pos=Point2D(-100, 0), radius=50),
            CircleState(pos=Point2D(100, 0), radius=50),
        ]
        states2 = [
...
@pytest.mark.integration
class TestFieldKeystates:
    """Test field -level keystate overrides"""

    def test_field_keystate_override(self):
        """Test field keystate overriding element keystates"""
        states = [
            CircleState(Point2D(), radius=50, fill_color=Color("#FF0000")),
            CircleState(Point2D(100, 100), radius=50, fill_color=Color("#FF0000")),
        ]
...
class TestComplexAnimations:
    ...
    def test_morphing_shapes(self):
        ...
        states = [
            CircleState(Point2D(), radius=50),
            CircleState(Point2D(), radius=100),
            CircleState(Point2D(), radius=50),
        ]
    ...
    def test_rotation_animation(self):
        ...
        states = [
            RectangleState(Point2D(), width=100, height=50, rotation=350),
    ...
    def test_combined_transformations(self):
        ...
        states = [
            CircleState(Point2D(), radius=50, scale=0.5, opacity=0.0, rotation=0),
            CircleState(
                Point2D(100, 100), radius=75, scale=1.5, opacity=1.0, rotation=180
            ),
        ]
