...
from svan2d.primitive.vertex.vertex_point import VertexPoint
from svan2d.core.point2d import Point2D

from .base_vertex import VertexState


@renderer("svan2d.primitive.renderer.point.PointRenderer")
...
class PointState(VertexState):
    ...
    Examples:
        ...
        VElement(keystates=[
            ...
            PointState(pos=Point2D(100, 0), y=50, opacity=0.0)
        ])
    ...
    num_vertices: int = (
        128  # Number of vertices (all at same location, matches other shapes)
    )

    def _generate_contours(self) -> VertexContours:
        """Generate degenerate contour (all vertices at same point)"""
        point = VertexPoint(center=Point2D(), num_vertices=self.num_vertices)
