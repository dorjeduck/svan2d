...
The Svan2D development server provides live browser preview with automatic hot-reload for rapid animation development. Edit your Python animation code, save the file, and see updates instantly in your browser—no manual refresh needed.
...
from svan2d.component import CircleState
...
svan2d serve my_animation.py --port 8080 --frames 30

# Faster playback (30 FPS)
svan2d serve my_animation.py --fps 30

# Smooth animation with many frames
svan2d serve my_animation.py --frames 60 --fps 30

# Don't open browser automatically
svan2d serve my_animation.py --no-browser
```

## Configuration

Configure defaults in `svan2d.toml`:
...
4. **Jupyter Preview** - Reuses Svan2D's Jupyter notebook preview system for consistency
...
1. **Use logging**: Set `logging.level = "DEBUG"` in `svan2d.toml` to see detailed server logs
...
```bash
svan2d serve my_animation.py --port 8001
```
...
- Configurable color schemes via `[preview]` section in `svan2d.toml`
...
- **[README.md](README.md)** - Main Svan2D documentation
