...
Used for clip_states, mask_states, and general List[State] attributes.
"""
...
class ShapeListInterpolator:
    ...
    def interpolate_state_list(
        ...
        # Apply M→N matching
        matched_loops1, matched_loops2 = vertex_loop_mapper.map(start_loops, end_loops)
    ...
    def _state_to_vertex_loop(self, state: State) -> VertexLoop:
        ...
        if isinstance(state, VertexState):
            ...
            translated_vertices = [
                Point2D(v.x + state.x, v.y + state.y) for v in contours.outer.vertices
            ]
    ...
    def _loops_to_states(
        self, matched_loops: List[VertexLoop], original_states: List[State]
    ) -> List[State]:
    ...
    def _create_zero_state(self, loop: VertexLoop) -> State:
        ...
        """
        from svan2d.component.state.point import PointState

        centroid = loop.centroid()
        return PointState(x=centroid.x, y=centroid.y, opacity=0.0)
