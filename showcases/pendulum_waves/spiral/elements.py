"""Element construction for the spiral pendulum waves showcase."""
...
def create_pendulum_elements(
    ...
    freq_inward: bool = False,
    start_angle: float = 0.0,
    clockwise: bool = True,
) -> list[VElement]:
    """Create pendulum VElements along an Archimedean spiral, hanging radially outward.
    ...
    for i in range(count):
        ...
        angle = direction * 2 * math.pi * turns * frac + start_rad
        radius = inner_radius + (outer_radius - inner_radius) * frac
        freq_i = (count - 1 - i) if freq_inward else i
        specs.append(
            PendulumSpec(
                pivot=Point2D(radius * math.cos(angle), radius * math.sin(angle)),
                orientation=angle,
                freq=base_oscillations + freq_i * freq_step,
            )
        )
