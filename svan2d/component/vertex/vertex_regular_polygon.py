...
class VertexRegularPolygon(VertexLoop):
    ...
    def __init__(
        ...
        Args:
            ...
            rotation: Rotation in degrees (0° = North, 90° = East, Svan2D convention)
        """
        if num_sides < 3:
            raise ValueError(f"Polygon requires at least 3 sides, got {num_sides}")
        if num_vertices < num_sides:
            raise ValueError(
                f"num_vertices ({num_vertices}) must be at least num_sides ({num_sides})"
            )
        ...
        side_lengths = [
            distance(corners[i], corners[(i + 1) % num_sides]) for i in range(num_sides)
        ]
