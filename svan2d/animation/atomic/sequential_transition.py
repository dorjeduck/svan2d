...
def sequential_transition(
    ...
    **transition_kwargs: Any,
    ...
    Example:
        >>> from svan2D.animations.atomic import fade, rotate, slide
        >>> from svan2D.animations.atomic.slide import SlideDirection
        >>>
    ...
    unique_keystates = [
        KeyState(time=max(0.0, min(1.0, ks.time)), state=ks.state)
        for ks in all_keystates
    ]
