# ============================================================================
# svan2D/animations/atomic/scale.py
# ============================================================================
...
def scale(
    ...
    """Scale down first state, switch attributes, scale up second state

    Element scales down to minimum (typically 0), attributes change,
    ...
    Example:
        >>> from svan2D.animations.atomic import scale
        >>>
    ...
    keystates = [
        ...
        KeyState(
            time=at_time, state=replace(state1, scale=min_scale, opacity=0.0)
        ),  # Scaled down
        KeyState(
            time=at_time, state=replace(state2, scale=min_scale, opacity=0.0)
        ),  # Switch (tiny)
        KeyState(
            time=t_end, state=replace(state2, scale=1.0, opacity=1.0)
        ),  # Scaled up
    ]
