...
class TestStaticExport:
    ...
    def test_export_multiple_shapes_svg(self, temp_export_dir):
        ...
        elements = [
            VElement(state=CircleState(pos=Point2D(-100, 0), radius=40)),
            VElement(state=RectangleState(width=60, height=60)),
            VElement(
                state=StarState(
                    pos=Point2D(100, 0),
                    outer_radius=50,
    ...
    def test_export_with_text_svg(self, temp_export_dir):
        ...
        elements = [
            VElement(state=CircleState(Point2D(0, -50), radius=40)),
            VElement(
                state=TextState(Point2D(0, 50), text="Hello Svan2D", font_size=24)
            ),
        ]
    ...
    def test_export_png_cairo(self, temp_export_dir):
        ...
        try:
            exporter = VSceneExporter(
                scene, output_dir=str(temp_export_dir), converter=ConverterType.CAIROSVG
            )
...
class TestComplexExportScenarios:
    ...
    def test_export_layered_composition(self, temp_export_dir):
        ...
        shapes = [
            VElement(
                state=CircleState(
                    pos=Point2D(-100, -50), radius=40, fill_color=Color("#FF0000")
                )
            ),
            VElement(
                state=RectangleState(
                    pos=Point2D(), width=60, height=60, fill_color=Color("#00FF00")
                )
            ),
            VElement(
                state=StarState(
        ...
        text = VElement(
            state=TextState(
                pos=Point2D(0, -120), text="Layered Composition", font_size=24
            )
        )
