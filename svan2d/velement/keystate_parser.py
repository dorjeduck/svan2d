"""Keystate parsing utilities for element and field timelines
...
def parse_attribute_keystates(
    ...
    - ALWAYS extends to cover 0.0-1.0 range (attributes exist when element exists)
    ...
    """
    if not flexible_list:
        raise ValueError("field keystates cannot be empty")
    ...
    for item in flexible_list:
        ...
        if not isinstance(item, tuple):
            ...
            easing = item[2]
            if not (0.0 <= t <= 1.0):
                raise ValueError(f"Attribute time must be between 0.0 and 1.0, got {t}")
            has_explicit_times = True
        else:
            raise ValueError(
                f"Attribute keyframe must be 1, 2 or 3 elements, got {len(item)}"
            )
    ...
    # Step 2: ALWAYS apply full timeline boundaries for attributes
    ...
    timestamped = _distribute_implicit_times_field(normalized)

    # Step 4: Sort and deduplicate
    return _finalize_attribute_keystates(timestamped)
