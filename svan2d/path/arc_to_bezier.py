...
def arc_to_beziers(
    ...
    try:
        ...
        center, rx, ry, start_angle, delta_angle, rot_rad = _get_center_and_angles(
            pos1, pos2, rx, ry, x_axis_rotation, large_arc_flag, sweep_flag
        )
    except Exception:
...
def _get_center_and_angles(
    ...
    x1_prime = cos_rot * dp.x + sin_rot * dp.y
    y1_prime = -sin_rot * dp.x + cos_rot * dp.y
    ...
    cx = cos_rot * cx_prime - sin_rot * cy_prime + (pos1.x + pos2.x) / 2
    cy = sin_rot * cx_prime + cos_rot * cy_prime + (pos1.y + pos2.y) / 2
    ...
    return Point2D(cx, cy), rx, ry, theta1, delta_theta, x_axis_rotation_rad
