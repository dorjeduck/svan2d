...
class VertexAstroid(VertexLoop):
    ...
    def __init__(
        ...
        if num_vertices < num_cusps:
            raise ValueError(
                f"num_vertices ({num_vertices}) must be at least num_cusps ({num_cusps})"
            )
    ...
    def _generate_astroid_vertices(
        self, cusps: Points2D, curvature: float, num_vertices: int, center: Point2D
    ) -> Points2D:
        """Generate vertices along the astroid curve
        ...
        for i in range(num_cusps):
            ...
            for j in range(vertices_per_segment):
                ...
                x = (
                    one_minus_t**2 * start_cusp.x
                    + 2 * one_minus_t * t * control_x
                    + t**2 * end_cusp.x
                )

                y = (
                    one_minus_t**2 * start_cusp.y
