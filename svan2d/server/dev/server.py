...
class DevServer:
    ...
    def _create_app(self) -> FastAPI:
        ...
        async def export_mp4(request: ExportRequest):
            ...
            asyncio.create_task(
                self._run_mp4_export(
                    job.job_id, request.frames, request.fps, request.width_px
                )
            )
        ...
        async def export_html(request: ExportRequest):
            ...
            asyncio.create_task(
                self._run_html_export(
                    job.job_id, request.frames, request.fps, request.html_interactive
                )
            )
    ...
    async def _run_batch_export(
        ...
        try:
            ...
            if needs_frames:
                ...
                logger.info(
                    f"Generating {total_frames} frames once for {len(formats)} format(s)..."
                )
                ...
                def progress_callback(frame_num: int, total: int):
                    ...
                    progress = (
                        frame_num / total
                    ) * 0.5  # Frame generation is 50% of total
                    for _, job_id in formats:
                        self.export_manager.update_job(
                            job_id,
                            progress=progress,
    ...
    def _encode_gif(
        self, frames_dir: Path, output_path: Path, total_frames: int, fps: int
    ):
    ...
    def _on_file_changed(self):
        ...
        if self.loop and self.loop.is_running():
            asyncio.run_coroutine_threadsafe(self._send_current_preview(), self.loop)
