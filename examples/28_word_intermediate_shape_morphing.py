...
def main():
    ...
    intermediate_states = [
        CircleState(
            fill_color=Color("#E57C04"),
            radius=20,
            pos=Point2D(-80 + i * 40, 0),
        )
        for i in range(5)
    ]

    # inter = StateCollectionState(states=intermediate_states)
    ...
    element = (
        ...
        # .keystate(inter, at=0.25)
        # .transition(morphing_config=morphing_config)
        .keystate(word2, at=0.4)
        .keystate(word2, at=0.6)
        # .transition(morphing_config=morphing_config)
        # .keystate(inter, at=0.75)
        .transition(morphing_config=morphing_config)
    ...
    exporter.to_mp4(
        filename="28_word_intermediate_shape_morphing",
        total_frames=120,
        framerate=30,
        png_width_px=1024,
    )
