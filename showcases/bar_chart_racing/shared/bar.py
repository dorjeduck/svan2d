"""Shared bar chart race elements — used by brand_values and ev_sales showcases."""
...
class CompanyDataPoint:
    """Single data point for a company/country at a specific time."""
...
class BarConfig:
    ...
    label_gap: float = 2.0       # Vertical gap between name and value labels
    label_reserve: float = 0.0   # Right-side space reserved for external labels
    label_h_offset: float = -5.0 # Horizontal offset from bar right edge (negative = inside bar)
    label_anchor: str = "end"    # Text anchor for labels ("end" = inside, "start" = outside)
...
def create_label_states(
    ...
    bar_width = max(1, config.bar_width * data_point.ratio_of_top)
    label_x = config.plot_left + bar_width + config.label_h_offset
    ...
    name_state = TextState(
        ...
        text_anchor=config.label_anchor,
        z_index=2,
    )
...
def estimate_label_reserve(
    ...
    Only considers entries that reach rank #1, since only the widest bars
    push labels near the right edge.
    """
    top_names = {
        name
