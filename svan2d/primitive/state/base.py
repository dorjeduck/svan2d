...
from svan2d.primitive.effect.filter import Filter
...
class State(ABC):
    ...
    # Cache marker: True iff this state will not change visually for any later frame.
    # Auto-set by StateInterpolator when t is past the final keystate and all
    # attribute timelines are exhausted; can be set manually by frame_fn callables
    # when they settle. Excluded from equality so two states differing only in
    # this flag still compare equal (preserves segment-constant fast path).
