...
class TestVertexBasedStates:
    ...
    def test_circle_state_creation(self):
        ...
        state = CircleState(Point2D(100, 100), radius=50)
        assert state.x == 100
        assert state.y == 100
    ...
    def test_circle_state_replace(self):
        """Test replacing CircleState attributes"""
        state = CircleState(Point2D(100, 100), radius=50)
        new_state = replace(state, pos=Point2D(200, 0), radius=75)
        assert new_state.pos.x == 200
        assert new_state.radius == 75
        assert new_state.pos.y == 0  # Unchanged
        assert state.pos.x == 100  # Original unchanged

    def test_ellipse_state_creation(self):
        """Test EllipseState creation"""
        state = EllipseState(Point2D(100, 100), rx=80, ry=50)
        assert state.rx == 80
        assert state.ry == 50

    def test_rectangle_state_creation(self):
        """Test RectangleState creation"""
        state = RectangleState(Point2D(100, 100), width=150, height=80)
        assert state.width == 150
        assert state.height == 80

    def test_square_state_creation(self):
        """Test SquareState creation"""
        state = SquareState(Point2D(100, 100), size=100)
        assert state.size == 100

    def test_triangle_state_creation(self):
        """Test TriangleState creation"""
        state = TriangleState(Point2D(100, 100), size=100)
        assert state.size == 100

    def test_polygon_state_creation(self):
        """Test PolygonState creation"""
        state = PolygonState(Point2D(100, 100), size=50, num_sides=6)
    ...
    def test_star_state_creation(self):
        ...
        state = StarState(
            Point2D(100, 100), outer_radius=80, inner_radius=40, num_points_star=5
        )
    ...
    def test_astroid_state_creation(self):
        ...
        state = AstroidState(Point2D(100, 100), radius=50)
        assert state.radius == 50

    def test_heart_state_creation(self):
        """Test HeartState creation"""
        state = HeartState(Point2D(100, 100), size=50)
        assert state.size == 50

    def test_infinity_state_creation(self):
        """Test InfinityState creation"""
        state = InfinityState(Point2D(100, 100), size=50)
        assert state.size == 50

    def test_spiral_state_creation(self):
        """Test SpiralState creation"""
        state = SpiralState(Point2D(100, 100), start_radius=10, end_radius=100, turns=3)
    ...
    def test_wave_state_creation(self):
        ...
        state = WaveState(Point2D(100, 100), length=200, amplitude=30, frequency=2)
    ...
    def test_flower_state_creation(self):
        ...
        state = FlowerState(Point2D(100, 100), size=50, num_petals=6)
        assert state.size == 50
...
class TestPerforatedStates:
    ...
    def test_ring_state_creation(self):
        ...
        state = RingState(Point2D(100, 100), outer_radius=80, inner_radius=50)
        assert state.outer_radius == 80
    ...
    def test_poly_ring_state_creation(self):
        ...
        state = PolyRingState(
            Point2D(100, 100), outer_size=80, inner_size=50, num_edges=6
        )
    ...
    def test_square_ring_state_creation(self):
        ...
        state = SquareRingState(Point2D(100, 100), outer_size=100, inner_size=60)
    ...
    def test_radial_segments_state_creation(self):
        ...
        state = RadialSegmentsState(Point2D(100, 100), num_lines=8)
...
class TestSimpleStates:
    ...
    def test_text_state_creation(self):
        ...
        state = TextState(Point2D(100, 100), text="Hello", font_size=24)
        assert state.text == "Hello"
        assert state.font_size == 24

    def test_arc_state_creation(self):
        """Test ArcState creation"""
        state = ArcState(Point2D(100, 100), radius=50, start_angle=0, end_angle=180)
        assert state.radius == 50
    ...
    def test_raw_svg_state_creation(self):
        ...
        state = RawSvgState(Point2D(), svg_data=svg_content)
        assert state.svg_data == svg_content


@pytest.mark.unit
class TestStateCommonFields:
    """Test common attributes across all states"""
    ...
    def test_common_position_fields(self, state_class, kwargs):
        """Test that all states have x, y position attributes"""
        state = state_class(pos=Point2D(123, 456), **kwargs)
        assert state.x == 123
        assert state.y == 456
    ...
    def test_common_transform_fields(self, state_class, kwargs):
        """Test that all states have scale, rotation, opacity"""
        state = state_class(Point2D(), scale=2.5, rotation=45, opacity=0.7, **kwargs)
    ...
    def test_common_visual_fields(self, state_class, kwargs):
...
class TestStateColorHandling:
    """Test color field handling in states"""
...
class TestStateEquality:
    ...
    def test_identical_states_equal(self):
        ...
        state1 = CircleState(Point2D(100, 100), radius=50)
    ...
    def test_different_values_not_equal(self):
        ...
        state1 = CircleState(Point2D(100, 100), radius=50)
        state2 = CircleState(Point2D(100, 100), radius=75)
        assert state1 != state2

    def test_different_types_not_equal(self):
        """Test that different state types are not equal"""
        state1 = CircleState(Point2D(100, 100), radius=50)
        state2 = RectangleState(Point2D(100, 100), width=100, height=100)
        assert state1 != state2
...
class TestStateHashing:
    ...
    def test_state_hashable(self):
        ...
        state1 = CircleState(Point2D(100, 100), radius=50)
        state2 = CircleState(Point2D(100, 100), radius=50)
        state3 = CircleState(pos=Point2D(200, 200), radius=75)
