# Svan2D Configuration Guide

Svan2d uses TOML-based configuration files to customize default values globally. This allows you to set project-wide preferences without repeating them in every function call.

## Quick Start

Create a `svan2d.toml` file in your project directory:
...
That's it! The config is automatically discovered and used by svan2d.

## Configuration File Locations

Svan2d automatically searches for configuration files in the following locations (in priority order):

1. **`./svan2d.toml`** - Project directory (highest priority)
2. **`~/.config/svan2d/config.toml`** - User config directory
3. **`~/.svan2d.toml`** - Home directory
4. **System defaults** - Built-in `svan2d/config/defaults.toml`
...
Simply create a `svan2d.toml` file with the options you want to customize:
...
create_config_template('svan2d.toml')
```

Or from command line:
```bash
python -c "from svan2d.config import create_config_template; create_config_template()"
```

This creates a `svan2d.toml` file with all available options and their default values.
...
### `[state.visual]` - Vertex-Based Shape Attributes

Default visual attributes for vertex-based shapes (CircleState, EllipseState, StarState, PolygonState, etc.).
...
**Usage:**
```python
from svan2d.component.state import PerforatedShapeState
...
```python
from svan2d.transition.interpolation.align_vertices import get_aligned_vertices
from svan2d.transition.interpolation.hole_matching import HungarianMapper
...
from svan2d.transition.interpolation.hole_matching import SimpleMapper, DiscreteMapper
...
```python
from svan2d.velement import VElement, KeyState
from svan2d.transition.interpolation.hole_matching import SimpleMapper
from svan2d.transition import easing
...
```bash
svan2d playwright-server start    # Start server in background
svan2d playwright-server stop     # Stop server
svan2d playwright-server status   # Check if running
svan2d playwright-server logs     # View recent logs
svan2d playwright-server restart  # Restart server
```
...
```bash
project-a/
├── svan2d.toml          # Project A defaults
└── animation.py

project-b/
├── svan2d.toml          # Project B defaults
└── animation.py
```

Each project automatically uses its own `svan2d.toml` when you run scripts from that directory.

## Best Practices

1. **Keep it minimal** - Only override values you actually need to change
2. **One per project** - Use `./svan2d.toml` in each project directory
3. **Version control** - Commit `svan2d.toml` to share settings with team
...
When the same value is defined in multiple places, svan2d uses this priority order:
...
3. **`./svan2d.toml`** (project directory)

4. **`~/.config/svan2d/config.toml`** (user config)

5. **`~/.svan2d.toml`** (home directory)
...
- Check file is named exactly `svan2d.toml`
...
- [`svan2d/config/defaults.toml`](svan2d/config/defaults.toml) - System defaults reference
