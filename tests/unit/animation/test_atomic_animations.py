...
from svan2d.animation.atomic import fade, scale, rotate, slide, pop, step, trim
...
class TestFadeAnimation:
    ...
    def test_fade_preserves_other_fields(self):
        """Test that fade preserves non-opacity attributes"""
        state1 = CircleState(
            Point2D(100, 100), radius=50, fill_color=Color("#FF0000"), rotation=45
        )
        state2 = CircleState(
            Point2D(100, 100), radius=50, fill_color=Color("#00FF00"), rotation=45
        )
        keystates = fade(state1, state2, duration=0.2)
...
class TestScaleAnimation:
    ...
    def test_scale_with_custom_min(self):
        ...
        keystates = scale(state1, state2, min_scale=0.5, duration=0.3)
...
class TestSlideAnimation:
    ...
    def test_slide_left_basic(self):
        ...
        state1 = CircleState(pos=Point2D(100, 0), radius=50)
        state2 = CircleState(Point2D(), radius=50)
    ...
    def test_slide_up_basic(self):
        ...
        state1 = CircleState(Point2D(), radius=50)
        state2 = CircleState(pos=Point2D(0, 100), radius=50)
    ...
    def test_slide_down_basic(self):
        ...
        state1 = CircleState(pos=Point2D(0, 100), radius=50)
        state2 = CircleState(pos=Point2D(0, 50), radius=50)
...
class TestStepAnimation:
    ...
    def test_step_basic(self):
        ...
        state2 = CircleState(Point2D(100, 100), radius=75, fill_color=Color("#0000FF"))
...
class TestAnimationComposition:
    ...
    def test_sequential_animations(self):
        ...
        from svan2d.animation.atomic import sequential_transition

        state1 = CircleState(Point2D(), radius=50)
        state2 = CircleState(pos=Point2D(100, 0), radius=50)
        state3 = CircleState(Point2D(100, 100), radius=50)

        keystates = sequential_transition(
            states=[state1, state2, state3], transition_func=fade, transition_factor=0.5
        )
...
class TestAnimationPreservation:
    """Test that animations preserve attributes correctly"""

    def test_fade_preserves_position(self):
        """Test that fade animation preserves position"""
        state1 = CircleState(Point2D(123, 456), radius=50)
        state2 = CircleState(Point2D(123, 456), radius=50)
        keystates = fade(state1, state2, duration=0.2)
    ...
    def test_slide_preserves_scale(self):
        ...
        state1 = CircleState(Point2D(), radius=50, scale=2.0)
        state2 = CircleState(pos=Point2D(100, 0), radius=50, scale=2.0)
        keystates = slide(state1, state2, direction="right", distance=100, duration=0.3)
    ...
    def test_rotate_preserves_color(self):
        ...
        state2 = RectangleState(width=100, height=60, fill_color=Color("#FF0000"))
...
class TestAnimationEdgeCases:
    ...
    def test_animation_with_none_state_fields(self):
        """Test animations with Color.NONE"""
        state1 = CircleState(radius=50, fill_color=Color.NONE, stroke_color=Color.NONE)
        state2 = CircleState(radius=50, fill_color=Color.NONE, stroke_color=Color.NONE)
        keystates = scale(state1, state2, duration=0.3)
