...
class TestStateInterpolation:
    ...
    def test_interpolate_circle_radius(self, interpolation_engine):
        ...
        state1 = CircleState(Point2D(), radius=50)
        state2 = CircleState(Point2D(), radius=100)
    ...
    def test_interpolate_rotation(self, interpolation_engine):
        ...
        state1 = RectangleState(Point2D(), width=100, height=50, rotation=0)
        state2 = RectangleState(Point2D(), width=100, height=50, rotation=180)
    ...
    def test_interpolate_scale(self, interpolation_engine):
        ...
        state1 = CircleState(Point2D(), radius=50, scale=1.0)
        state2 = CircleState(Point2D(), radius=50, scale=2.0)
...
class TestVertexInterpolation:
    ...
    def test_interpolate_vertex_contours(self, interpolation_engine):
        ...
        result = interpolation_engine.interpolate_value(
            start_state=CircleState(Point2D(), radius=50),  # Dummy state
            end_state=CircleState(Point2D(), radius=50),
    ...
    def test_interpolate_mismatched_vertex_counts_raises(self, interpolation_engine):
        ...
        with pytest.raises(ValueError, match="Vertex lists must have same length"):
            interpolation_engine.interpolate_value(
                start_state=CircleState(Point2D(), radius=50),
                end_state=CircleState(Point2D(), radius=50),
                field_name="_aligned_contours",
                start_value=contours1,
                end_value=contours2,
...
class TestEdgeCases:
    ...
    def test_interpolate_at_t_zero(self, interpolation_engine):
        """Test interpolation at t=0 returns start state attributes"""
        state1 = CircleState(Point2D(), radius=50, fill_color=Color("#FF0000"))
        state2 = CircleState(Point2D(100, 100), radius=100, fill_color=Color("#0000FF"))
    ...
    def test_interpolate_at_t_one(self, interpolation_engine):
        """Test interpolation at t=1 returns end state attributes"""
        state1 = CircleState(Point2D(), radius=50, fill_color=Color("#FF0000"))
        state2 = CircleState(Point2D(100, 100), radius=100, fill_color=Color("#0000FF"))
    ...
    def test_interpolate_identical_states(self, interpolation_engine):
        ...
        state1 = CircleState(pos=Point2D(50, 50), radius=50)
        state2 = CircleState(pos=Point2D(50, 50), radius=50)
    ...
    def test_attribute_keystates_attributes_skipped(self, interpolation_engine):
        """Test that attributes in attribute_keystates_fields are skipped"""
        state1 = CircleState(Point2D(), radius=50)
        state2 = CircleState(Point2D(100, 50), radius=50)

        # Mark 'x' as managed by field keystates
        result = interpolation_engine.create_eased_state(
            state1,
            state2,
            t=0.5,
            segment_easing_overrides=None,
            attribute_keystates_fields={"pos"},
        )

        # x should NOT be interpolated (it's managed elsewhere)
        # Result should come from start_state for attributes not interpolated
        # But y should be interpolated normally
        assert result.y == 50

    def test_non_interpolatable_attributes_step_function(self, interpolation_engine):
        """Test that NON_INTERPOLATABLE_FIELDS use step function"""
        # NON_INTERPOLATABLE_FIELDS should switch at t=0.5
        # We need to check what attributes are non-interpolatable
        # For now, let's assume structural attributes like 'closed' are non-interpolatable

        # This is a conceptual test - actual implementation depends on
        # which attributes are marked as NON_INTERPOLATABLE_FIELDS
        pass
