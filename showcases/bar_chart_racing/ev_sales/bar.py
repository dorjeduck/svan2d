...
class BarConfig:
    ...
    label_reserve: float = 0.0  # Right-side space reserved for labels
    ...
    def bar_width(self) -> float:
        """Maximum bar width based on plot area minus label reserve."""
        return self.plot_right - self.plot_left - self.label_reserve
...
def estimate_label_reserve(
    company_data: dict[str, list[CompanyDataPoint]],
    name_font_size: int,
    value_font_size: int,
    label_gap: float,
    name_char_width_factor: float = 0.45,
    value_char_width_factor: float = 0.65,
) -> float:
    """Estimate the pixel width needed for labels to the right of bars.

    Only considers countries that actually reach rank #1, since only
    the widest bars push labels near the right edge.

    Args:
        company_data: Dictionary mapping names to data points
        name_font_size: Font size for name labels
        value_font_size: Font size for value labels
        label_gap: Gap between bar end and label start
        name_char_width_factor: Char width factor for proportional name font
        value_char_width_factor: Char width factor for monospace value font

    Returns:
        Estimated pixel reserve for the label area
    """
    # Find names that reach rank #1
    top_names = set()
    for name, data_points in company_data.items():
        if any(p.rank == 1 for p in data_points):
            top_names.add(name)

    max_name_width = max(
        (len(name) * name_font_size * name_char_width_factor for name in top_names),
        default=0.0,
    )

    # Value width based on the global max (rank #1 has the largest values)
    max_val = max(
        p.value for data_points in company_data.values() for p in data_points
    )
    max_value_width = len(f"{max_val:,.0f}") * value_font_size * value_char_width_factor

    return max(max_name_width, max_value_width) + label_gap
